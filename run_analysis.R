...
set_merge <- rbind(X_train, X_test)
...
subject_merge <- merge(subject_train,subject_test, all=TRUE, sort=FALSE)
...
tidy_data2 <- tidy_data2[order(as.numeric(tidy_data2$Subject)),] #Order the final dataset by subject number. 

#Writing a .txt file for the submission. 
write.table(tidy_data2, "./tidy_data.txt", row.names=FALSE)
